...
export default function LoanCalculator() {
  ...
  useEffect(() => {
    if (loanResult.maxLoanAmount > 0 && loanDuration) {
      generatePaymentSchedule()
    }
  }, [loanResult.maxLoanAmount, loanDuration])
  ...
  const updateCalculations = () => {
    ...
    if (loanDuration) {
      ...
      const finalAmount = maxYellowAmount > 0 ? maxYellowAmount : maxBlueAmount
      const finalEMI = maxYellowAmount > 0 ? maxYellowEMI : maxBlueEMI

      setLoanResult({
        maxDuration: maxDur,
        contractEnd: formatDate(end),
        maxLoanAmount: finalAmount,
        maxLoanEMI: finalEMI
      })
      setUiState(prev => ({ ...prev, showWarning: duration > maxDur }))
    } else {
      setLoanResult(prev => ({ ...prev, maxDuration: maxDur }))
    }
  }
  ...
  const generatePaymentSchedule = () => {
    if (!loanResult.maxLoanAmount || !loanDuration) return

    const duration = parseInt(loanDuration)
    const monthlyPayment = loanResult.maxLoanEMI
    let balance = loanResult.maxLoanAmount
      ...
      <div className="container mx-auto py-6 sm:py-8 px-4">
        ...
        <Card className="shadow-sm border-0 bg-white rounded-2xl overflow-hidden">
          ...
          <CardContent className="space-y-6 p-6">
            {/* Input Fields */}
            <div className="grid gap-5 md:grid-cols-[2fr_2fr]">
